...
export class WorldRenderer {
  ...
  buildCellMarks(mesh,model,solids,showBotMarks=true){const focusStart=Math.max(0,Math.min(14,this.focusLayer)),visible=z=>!this.layerFocus||z>=focusStart&&z<=focusStart+1,parse=encoded=>{const[x,y,z]=encoded.split(",").map(Number);return{x,y,z}};for(const encoded of model.safeMarks??[]){const cell=parse(encoded);if(!this.solid(solids,cell.x,cell.y,cell.z)||!visible(cell.z))continue;for(const g of this.cellFaces(cell,solids))mesh.planeFrame(add(g.center,mul(g.normal,.018)),g.u,g.v,.56,color(73,235,207,235),.055,g.normal)}for(const encoded of model.mineFlags??[]){const cell=parse(encoded);if(!this.solid(solids,cell.x,cell.y,cell.z)||!visible(cell.z))continue;for(const g of this.cellFaces(cell,solids)){const a=norm(add(g.u,g.v)),b=norm(sub(g.u,g.v)),center=add(g.center,mul(g.normal,.035)),red=color(247,73,97);mesh.planeBox(center,a,b,vec(.58,.075,.055),red);mesh.planeBox(center,b,a,vec(.58,.075,.055),red)}}if(!showBotMarks)return;for(const encoded of model.botSafeMarks??[]){const cell=parse(encoded);if(!this.solid(solids,cell.x,cell.y,cell.z)||!visible(cell.z))continue;for(const g of this.cellFaces(cell,solids))mesh.planeFrame(add(g.center,mul(g.normal,.022)),g.u,g.v,.43,color(86,169,255,235),.045,g.normal)}for(const encoded of model.botMineFlags??[]){const cell=parse(encoded);if(!this.solid(solids,cell.x,cell.y,cell.z)||!visible(cell.z))continue;for(const g of this.cellFaces(cell,solids)){const a=norm(add(g.u,g.v)),b=norm(sub(g.u,g.v)),center=add(g.center,mul(g.normal,.041)),orange=color(255,166,72);mesh.planeBox(center,a,b,vec(.43,.06,.05),orange);mesh.planeBox(center,b,a,vec(.43,.06,.05),orange)}}}
  ...
  render(model,solids,pieces,moves,mines=model.mines,revealedClues=model.revealedClues,showBotMarks=true){
    this.resize();const mesh=new MeshBuilder();this.buildTerrain(mesh,solids);this.buildCellMarks(mesh,model,solids,showBotMarks);this.buildClues(mesh,model,solids,mines,revealedClues);this.buildHints(mesh,model,moves);for(const p of [...pieces].sort((a,b)=>dot(sub(b.position,this.camera),sub(b.position,this.camera))-dot(sub(a.position,this.camera),sub(a.position,this.camera))))this.buildPiece(mesh,p,p.piece.id===model.selectedId);
