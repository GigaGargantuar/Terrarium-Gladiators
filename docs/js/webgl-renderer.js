...
export class WorldRenderer {
  ...
  cellGeometry(cell,solids=null){const center=vec(cell.x,cell.y,cell.z+.49),toward=sub(this.camera,center),normals=[vec(1,0,0),vec(-1,0,0),vec(0,1,0),vec(0,-1,0),vec(0,0,1),vec(0,0,-1)],renderedSolid=(x,y,z)=>this.solid(solids,x,y,z)&&(!this.layerFocus||z>=this.focusLayer&&z<=this.focusLayer+1),normal=normals.filter(n=>!solids||!renderedSolid(cell.x+n.x,cell.y+n.y,cell.z+n.z)).sort((a,b)=>dot(b,toward)-dot(a,toward))[0]??normals.sort((a,b)=>dot(b,toward)-dot(a,toward))[0];let u,v;if(normal.x){u=vec(0,normal.x>0?1:-1,0);v=vec(0,0,1)}else if(normal.y){u=vec(normal.y>0?-1:1,0,0);v=vec(0,0,1)}else{u=vec(1,0,0);v=vec(0,normal.z>0?1:-1,0)}return{center:add(center,mul(normal,.505)),u,v,normal}}
  cellPoint(cell,solids){return this.cellGeometry(cell,solids).center}
  ...
  buildCellMarks(mesh,model,solids){const focusStart=Math.max(0,Math.min(14,this.focusLayer)),visible=z=>!this.layerFocus||z>=focusStart&&z<=focusStart+1,parse=encoded=>{const[x,y,z]=encoded.split(",").map(Number);return{x,y,z}};for(const encoded of model.safeMarks??[]){const cell=parse(encoded);if(!this.solid(solids,cell.x,cell.y,cell.z)||!visible(cell.z))continue;const g=this.cellGeometry(cell,solids);mesh.planeFrame(add(g.center,mul(g.normal,.018)),g.u,g.v,.56,color(73,235,207,235),.055,g.normal)}for(const encoded of model.mineFlags??[]){const cell=parse(encoded);if(!this.solid(solids,cell.x,cell.y,cell.z)||!visible(cell.z))continue;const g=this.cellGeometry(cell,solids),a=norm(add(g.u,g.v)),b=norm(sub(g.u,g.v)),center=add(g.center,mul(g.normal,.035)),red=color(247,73,97);mesh.planeBox(center,a,b,vec(.58,.075,.055),red);mesh.planeBox(center,b,a,vec(.58,.075,.055),red)}}
